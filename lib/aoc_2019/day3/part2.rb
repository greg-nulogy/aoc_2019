  ...
  module Day3
    class Part2
      ...
      def calculate_distance_of_closest_cross(wire_paths_coordinates)
        ...
        closest_cross = wire_cross_coordinates.min do |a, b|
          (
            wire_paths_coordinates[0][:coordinates].find { |coord_a| coord_a == a }.index +
            wire_paths_coordinates[1][:coordinates].find { |coord_a| coord_a == a }.index
          ) <=> (
            wire_paths_coordinates[0][:coordinates].find { |coord_b| coord_b == b }.index +
            wire_paths_coordinates[1][:coordinates].find { |coord_b| coord_b == b }.index
          )
        end

        wire_paths_coordinates[0][:coordinates].find { |coord_a| coord_a == closest_cross }.index +
          wire_paths_coordinates[1][:coordinates].find { |coord_a| coord_a == closest_cross }.index
      end
    end

    WireSegment = Struct.new(:x, :y, :index) do
      def self.origin
        WireSegment.new(0, 0, 0)
      end
